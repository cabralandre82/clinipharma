...
_Última atualização: 2026-04-19 — Wave 6 concluída (observabilidade 3 camadas: `/api/health/{live,ready,deep}` + alias legado, `lib/metrics.ts` (counters/gauges/histograms + surge detector + exposição Prometheus), `lib/alerts.ts` (PagerDuty Events v2 + email Resend com dedup/cooldown), `lib/security/client-csrf.ts` (fetchWithCsrf + hook `useCsrfToken` viabilizando `CSRF_ENFORCE_DOUBLE_SUBMIT=true`), migration 048 seedando `alerts.pagerduty_enabled`/`alerts.email_enabled` (ambos OFF) em staging+prod, wiring de métricas em CSRF middleware / RBAC / webhooks/dedup / cron/guarded / circuit-breaker, alerta P2 automático em surge de `rbac_rpc_errors`, 40 unit tests novos + smoke E2E de health, runbooks `health-check-failing.md` + `alerts-noisy.md`)._
